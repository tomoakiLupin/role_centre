...
class CommandRegistry {
    ...
    loadCommands(commandsPath) {
        if (!fs.existsSync(commandsPath)) {
            console.warn(`[command_registry]命令目录未找到: ${commandsPath}跳过命令加载`);
            return;
        }
    ...
    loadHandlers(handlersPath) {
        if (!fs.existsSync(handlersPath)) {
            console.warn(`[command_registry]处理器目录未找到: ${handlersPath}跳过处理器加载`);
            return;
        }
            ...
            if (entry.isDirectory()) {
                ...
                try {
                    ...
                    if (handler.commandName) {
                        ...
                        console.log(`[command_registry]已注册处理器对应命令: ${handler.commandName}`);
                    } else {
                        console.log(`[command_registry]处理器文件 ${entry.name} 未导出 'commandName'它可能不是一个斜杠命令处理器`);
                    }
                } catch (error) {
                    console.error(`[command_registry]从 ${entry.name} 加载处理器失败:`, error);
                }
            }
        }
    }
